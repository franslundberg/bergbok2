...
CREATE TABLE IF NOT EXISTS bookkeeping_runs (id TEXT PRIMARY KEY,company_id TEXT NOT NULL REFERENCES companies(id),period_id TEXT NOT NULL,job_id TEXT NOT NULL UNIQUE REFERENCES bookkeeping_jobs(id),outcome_kind TEXT NOT NULL,run_ref_json TEXT NOT NULL,run_sha256 TEXT NOT NULL,decision TEXT CHECK(decision IN ('approved','rejected')),decided_at INTEGER,superseded_at INTEGER,created_at INTEGER NOT NULL) STRICT;
